 ...
 * QuickTime movie.
 */
public class JpegImagesToMovieConverter implements ControllerListener, DataSinkListener, VideoConverter {
	...
	public boolean doIt(int width, int height, int frameRate, Vector<String> inFiles, MediaLocator outML)
		...
		try {
			...
			throw new Exception("Cannot create a processor from the data source.");
		}
		...
		try {
			...
			throw new Exception("IO error during processing");
		}
	 ...
	 * Create the DataSink.
	 */
	DataSink createDataSink(Processor p, MediaLocator outML) throws Exception {
		...
		try {
			...
			throw new Exception("Cannot create the DataSink: " + e);
		}
	...
	public JpegImagesToMovieConverter() {
		this.width = 500;
		this.height = 500;
		this.frameRate = 1;
		this.inputFiles = new Vector<String>();
		this.outputURL = SelfieProperties.VIDEO_PATH;
		this.rootDir = SelfieProperties.FILE_PATH;
	}
	...
	private void validate() throws Exception {
		...
		if (rootDir == null) {
			throw new Exception("No Images Directory Specified.");
		}
		...
		if (outputURL == null) {
			outputURL = (new File("E:\\upload\\videos\\")).getAbsolutePath() + File.separator + "selfie.mov";
		}
		...
		if ((this.oml = createMediaLocator(outputURL)) == null) {
			throw new Exception("Cannot build media locator from: " + outputURL);
		}
